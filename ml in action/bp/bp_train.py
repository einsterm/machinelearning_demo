...
def hidden_in(featureMat, w0, b0):
    '''计算隐含层的输入
    ...
    '''
    m = np.shape(featureMat)[0]
    hidden_in = featureMat * w0
    for i in xrange(m):
        hidden_in[i,] += b0
    return hidden_in
...
def bp_train(featureMat, label, n_hidden, maxCycle, alpha, n_output):
    '''计算隐含层的输入
    input:  feature(mat):特征
            label(mat):标签
    ...
    while i <= maxCycle:
        # 2.1、信号正向传播
        # 2.1.1、计算隐含层的输入
        Zi_2 = hidden_in(featureMat, w0, b0)  # mXn_hidden
        # 2.1.2、计算隐含层的输出
        Ai_2 = hidden_out(Zi_2)

        Ai_2 = np.mat([[0.4, 0.25, 0.3],
                       [0.6, 0.2, 0.1],
                       [0.6, 0.2, 0.3],
                       [0.6, 0.2, 0.4]])

        # 2.1.3、计算输出层的输入
        Zi_3 = predict_in(Ai_2, w1, b1)  # mXn_output
        # 2.1.4、计算输出层的输出
        Ai_3 = predict_out(Zi_3)
        ...
        # 2.2、误差的反向传播
        # 2.2.1、隐含层到输出层之间的残差
        delta_3 = -np.multiply((label - Ai_3), f_sigmoid(Zi_3))
        # 2.2.2、输入层到隐含层之间的残差
        delta_2 = np.multiply((delta_3 * w1.T), f_sigmoid(Zi_2))

        # 2.3、 修正权重和偏置       
        w1 = w1 - alpha * (Ai_2.T * delta_3)
        b1 = b1 - alpha * np.sum(delta_3, axis=0) * (1.0 / m)
        w0 = w0 - alpha * (featureMat.T * delta_2)
        b0 = b0 - alpha * np.sum(delta_2, axis=0) * (1.0 / m)
        # if i % 100 == 0:
        #     print "\t-------- iter: ", i, \
        #         " ,cost: ", (1.0 / 2) * get_cost(get_predict(featureMat, w0, w1, b0, b1) - label)
        i += 1
    return w0, w1, b0, b1
...
if __name__ == "__main__":
    # 1、导入数据
    print "--------- 1.load data ------------"
    feature, label, n_class = load_data("data10.txt")
    # 2、训练网络模型
    print "--------- 2.training ------------"
    w0, w1, b0, b1 = bp_train(feature, label, 3, 100, 0.01, n_class)
    # 3、保存最终的模型
    print "--------- 3.save model ------------"
