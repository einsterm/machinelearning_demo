...
def get_patch(input_array, i, j, filter_width, filter_height, stride):
    ...
    m = start_i + filter_height
    e = start_j + filter_width

    if input_array.ndim == 2:
        return input_array[start_i: m, start_j: e]
...
def conv(input_array, kernel_array, output_array, stride, bias):
    ...
    for i in range(output_height):
        for j in range(output_width):
            ...
            patchSum = patch * kernel_array
            str=[]
            str.append(patchSum[0].sum())
            str.append(patchSum[1].sum())
            str.append(patchSum[2].sum())
            print(str)
            print("-------------------------")
...
def element_wise_op(array, op):
    # nditer函数加readwrite可以修改array里的元素，具体按op方法修改
...
class ConvLayer(object):
    ...
    def bp_sensitivity_map(self, sensitivity_array, activator):
        ...
        # 执行完下面方法，derivative_array相当于f'(a) 即对激活函数求偏导的值
        element_wise_op(derivative_array, activator.backward)
        self.delta_array *= derivative_array  # 完成对w的偏导
    ...
    def bp_gradient(self, sensitivity_array):
        ...
        for f in range(self.filter_number):
            ...
            # 计算偏置项的梯度
            bias_Arr = expanded_array[f]
            filter.bias_grad = bias_Arr.sum()
    ...
    def create_delta_array(self):
        deltaArr = np.zeros((self.channel_number, self.input_height, self.input_width))
        return deltaArr
...
class MaxPoolingLayer(object):
    ...
    def forward(self, input_array):
        for d in range(self.channel_number):
            for i in range(self.output_height):
                for j in range(self.output_width):
                    patch = get_patch(input_array[d], i, j, self.filter_width, self.filter_height, self.stride).max()
                    self.output_array[d, i, j] = (patch)
